...
function printHelp() {
  console.log(`Uso:
  node .agents/skills/update.js [--dry-run] [--no-push]
...
function composeCommitMessage(type, scope, subject) {
  const normalizedScope = scope ? `(${scope})` : "";
  return `${type}${normalizedScope}: ${subject}`;
}
...
function commitRepo({ repoRoot, messageHeader, messageBody, dryRun }) {
  if (dryRun) {
    console.log(`\n[dry-run] git -C "${repoRoot}" add -A`);
    console.log(`[dry-run] git -C "${repoRoot}" commit -m "${messageHeader}"`);
    if (messageBody) {
      console.log(`[dry-run] segunda mensagem: "${messageBody}"`);
    }
    return;
  }
...
async function main() {
  ...
  try {
    ...
    const scope = await ask(rl, "Escopo (opcional, ex: project-init): ");
    const subject = await ask(rl, "Mensagem curta (obrigatoria): ", {
      required: true,
    });
    const body = await ask(rl, "Descricao adicional (opcional): ");

    const commitMessage = composeCommitMessage(type, scope, subject);
    console.log("\nCommit a ser criado:");
