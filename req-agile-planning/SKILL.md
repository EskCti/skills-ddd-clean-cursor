...
- Backend → `config-project` / `config-project-kt` / `config-project-cs` / `config-project-rs` / `config-project-java`
...
- Docker produção → `config-docker` / `config-docker-kt` / `config-docker-cs` / `config-docker-rs` / `config-docker-java`
- CI/CD → `config-cicd` / `config-cicd-kt` / `config-cicd-cs` / `config-cicd-rs` / `config-cicd-java`
- Shared kernel → `config-shared-core` / `config-shared-core-kt` / `config-shared-core-cs` / `config-shared-core-rs` / `config-shared-core-java`
- Banco → `config-prisma` / `config-jpa-kt` / `config-jpa-java` / `config-efcore-cs` / `config-sqlx-rs`
...
| Prefixo | Agent TS | Agent KT | Agent CS | Agent RS | Agent Java |
| ---------------------- | ----------------------- | -------------------------------- | ----------------------- | ----------------------- | ----------------------- |
| `domain:vo` | `Core Value Object` | `Core Value Object (Kotlin)` | `Core Value Object (C#)` | `Core Value Object (Rust)` | `Core Value Object (Java)` |
| `domain:entity` | `Core Entity` | `Core Entity (Kotlin)` | `Core Entity (C#)` | `Core Entity (Rust)` | `Core Entity (Java)` |
| `domain:service` | `Core Domain Service` | `Core Domain Service (Kotlin)` | `Core Domain Service (C#)` | `Core Domain Service (Rust)` | `Core Domain Service (Java)` |
| `domain:repository` | `Core Repository` | `Core Repository (Kotlin)` | `Core Repository (C#)` | `Core Repository (Rust)` | `Core Repository (Java)` |
| `app:dto` | `Core DTO` | `Core DTO (Kotlin)` | `Core DTO (C#)` | `Core DTO (Rust)` | `Core DTO (Java)` |
| `app:usecase` | `Core Use Case` | `Core Use Case (Kotlin)` | `Core Use Case (C#)` | `Core Use Case (Rust)` | `Core Use Case (Java)` |
| `app:query` | `Core Query CQRS` | `Core Query CQRS (Kotlin)` | `Core Query CQRS (C#)` | `Core Query CQRS (Rust)` | `Core Query CQRS (Java)` |
| `infra:persistence` | `Backend Prisma Data` | `Backend Data (Kotlin)` | `Backend Data (C#)` | `Backend Data (Rust)` | `Backend Data (Java)` |
| `infra:migration` | `Config Prisma` | `Config JPA (Kotlin)` | `Config EF Core (C#)` | `Config SQLx (Rust)` | `Config JPA (Java)` |
| `infra:setup` | `Config Project` | `Config Project (Kotlin)` | `Config Project (C#)` | `Config Project (Rust)` | `Config Project (Java)` |
| `infra:shell-web` | `Config Shared Web` / `(Angular)` / `(Vue)` | — | — | — | — |
| `domain:shared` | `Config Shared Core` | `Config Shared Core (Kotlin)` | `Config Shared Core (C#)` | `Config Shared Core (Rust)` | `Config Shared Core (Java)` |
| `infra:auth` | `Config Auth Core Basic` | `Config Auth Core Basic (Kotlin)` | `Config Auth Core (C#)` | — | — |
| `interface:controller` | `Backend Controller` | `Backend Controller (Kotlin)` | `Backend Controller (C#)` | `Backend Controller (Rust)` | `Backend Controller (Java)` |
...
| `infra:docker` | `Config Docker (TypeScript)` | `Config Docker (Kotlin)` | `Config Docker (C#)` | `Config Docker (Rust)` | `Config Docker (Java)` |
| `infra:cicd` | `Config CI/CD (TypeScript)` | `Config CI/CD (Kotlin)` | `Config CI/CD (C#)` | `Config CI/CD (Rust)` | `Config CI/CD (Java)` |
| `infra:fullstack` | `Config Project Full-Stack` | `Config Project Full-Stack` | `Config Project Full-Stack` | `Config Project Full-Stack` | `Config Project Full-Stack` |
