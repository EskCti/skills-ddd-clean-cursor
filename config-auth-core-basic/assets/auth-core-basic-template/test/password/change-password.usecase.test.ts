...
const VALID_HASH = '$2b$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy';
const ANOTHER_VALID_HASH = '$2b$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWx';
...
describe('ChangePasswordUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new ChangePasswordUseCase(mockPassRepo, mockUserExistsQuery, mockPasswordCryptoProvider);

    mockUserExistsQuery.execute.mockResolvedValue(Result.ok(true));
    mockPassRepo.findRecentByUserId.mockResolvedValue(Result.ok([Password.create({ content: VALID_HASH })]));
  ...
  test('should propagate failure when finding recent passwords fails', async () => {
    mockPassRepo.findRecentByUserId.mockResolvedValue(Result.fail('RECENT_PASSWORDS_ERROR'));
