---
name: prisma-init
description: "Inicializar e padronizar o Prisma no backend NestJS do Pharmacore com schema modular por domínio (`apps/backend/prisma/models/*.prisma`), entrypoint de seed em `apps/backend/prisma/seed/main.ts`, configuração de `prisma.config.ts`, Docker Compose do backend compatível com `DATABASE_URL` do `.env`, e criação/ajuste de `DbModule` + `PrismaService`. Usar quando o pedido envolver setup inicial de Prisma, onboarding de módulos com arquivo Prisma próprio ou rebootstrap da infraestrutura de banco no backend."
---
...
2. Executar o script da skill:
   - `node .agents/skills/prisma-init/scripts/init-prisma-backend.js --dry-run`
   - `node .agents/skills/prisma-init/scripts/init-prisma-backend.js --apply --install`
3. Incluir arquivos Prisma por módulo (repetível):
   - `node .agents/skills/prisma-init/scripts/init-prisma-backend.js --apply --module auth --module product`
...
- `apps/backend/prisma/models/bootstrap.prisma` temporário para manter bootstrap/migrate inicial funcionais.
...
- Após criar modelos reais, remover `prisma/models/bootstrap.prisma` e gerar nova migration.
