...
- schema modular em `apps/backend/prisma/models/*.prisma`
...
```bash
node .agents/skills/prisma-init/scripts/init-prisma-backend.js --apply --module auth --module stock --module billing
```
...
- Adicionar modelos reais em `prisma/models/*.prisma`
...
- Remover `prisma/models/bootstrap.prisma` após entrada dos modelos reais e gerar migration de substituição
