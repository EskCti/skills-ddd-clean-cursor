...
const TEMPLATE_DIR = path.join(__dirname, '..', 'assets', 'auth-core-basic-template-kt');

function usage() {
  console.log(`Usage:
  node create-auth-core-basic-kt.mjs [--group com.example] [--force] [--run-tests]

Examples:
  node create-auth-core-basic-kt.mjs
  node create-auth-core-basic-kt.mjs --group com.acme --force
  node create-auth-core-basic-kt.mjs --force --run-tests`);
}
...
async function detectTargetDir(rootDir) {
  ...
  if (await pathExists(path.join(rootDir, 'packages', 'auth', 'core'))) return nested;
...
async function updateSettingsGradle(rootDir, targetDir, ops, logger) {
  const settingsPath = path.join(rootDir, 'settings.gradle.kts');
  ...
  const content = await fs.readFile(settingsPath, 'utf8');
  if (content.includes(includeStr)) return;
...
async function main() {
  ...
  const logger = await createSkillRunLogger({
    rootDir,
    skillName: 'config-auth-core-basic-kt',
  ...
  try {
    ...
    if (await pathExists(targetDir)) {
      ...
      logger.step(`Removed existing directory with --force: ${targetDir}`);
    }

    const hasTemplate = await pathExists(TEMPLATE_DIR);
    if (hasTemplate) {
    ...
    console.log(`\n  Auth core basic (Kotlin) created at: ${targetDir}\n`);
