...
const toneClasses: Record<
  TableCardTone,
  { card: string; title: string; footer: string }
> = {
  default: {
    card: "",
    title: "",
  ...
  return (
    <Card className={cn("overflow-hidden", toneClasses[tone].card, className)}>
        ...
        >
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className={cn("space-y-1.5", !title && "space-y-0")}>
              {title ? (
                <CardTitle
                  className={cn("text-lg font-bold tracking-tight", toneClasses[tone].title)}
                >
                  {title}
                </CardTitle>
              ) : null}
            ...
            {headerAside ? (
              <div className="text-xs text-muted-foreground">{headerAside}</div>
            ) : null}
          </div>
        </CardHeader>
      ) : <div className="h-3" />}
      ...
      {footer ? (
        <div
          className={cn(
            "border-t px-5 py-4 md:px-6",
            toneClasses[tone].footer,
            footerClassName,
          )}
        >
          {footer}
