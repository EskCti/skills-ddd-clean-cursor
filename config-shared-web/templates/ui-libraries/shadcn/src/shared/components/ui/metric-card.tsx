    ...
    >
      ...
      <CardHeader className={cn("relative space-y-2 pb-2", headerClassName)}>
        <div className="flex items-start justify-between gap-2">
          <CardTitle
            className={cn("text-[15px] font-semibold text-zinc-100", titleClassName)}
          >
            {title}
          </CardTitle>
          ...
          {icon ? (
            <span
              className={cn(
                "rounded-md border border-white/10 bg-white/5 p-1.5 text-zinc-400",
                iconContainerClassName,
              )}
            >
              {icon}
        ...
        {subtitle ? (
          <p className={cn("text-xs text-zinc-400", subtitleClassName)}>{subtitle}</p>
        ) : null}
      </CardHeader>

      <CardContent className={cn("relative", contentClassName)}>
        <p className={cn("text-3xl md:text-4xl font-black tracking-tight text-zinc-100", valueClassName)}>
          {value}
        </p>
      </CardContent>
    </Card>
